...
func process(_delta : float) -> void:
	...
	match(assignment.goal_state):
		...
		Assignment.GOAL_STATE.BUILD:
			process_build_assignment()
		
		Assignment.GOAL_STATE.MINE:
			process_mine_assignemnt()
		
		Assignment.GOAL_STATE.RETURN:
			process_return_assignment()

func process_build_assignment() -> void:
	...
	if(!construction_assignment.picked_up):
		...
		if(entity.global_position.distance_squared_to(pickup_site.global_position) > pow(pickup_site.get_body_radius(), 2)):
			entity.move_state = Entity.MOVE_STATE.APPROACH
			entity.move_target = pickup_site
			entity.approach_distance = pickup_site.get_body_radius() * 0.9
		else:
		...
		if(entity.global_position.distance_squared_to(construction_site.global_position) > pow(construction_site.get_body_radius(), 2)):
			entity.move_state = Entity.MOVE_STATE.APPROACH
			entity.move_target = construction_site
			entity.approach_distance = construction_site.get_body_radius() * 0.9
		else:
...
func process_mine_assignemnt() -> void:
...
func process_return_assignment() -> void:
	...
	if(!target_entity_valid):
		...
		entity.approach_distance = (entity.get_body_radius() + target_entity.get_body_radius()) * 0.9
		...
		if(distance_sqr >= approach_distance_sqr + 0.0001):
			entity.move_state = Entity.MOVE_STATE.APPROACH
